...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=True, image_weights=False,
                 cache_labels=False, cache_images=False, keep_rate = None):

        LABEL_FOLDER = 'labels_EXP1_Original'
        ...
        # Define labels
        self.label_files = [x.replace('JPEGImages', LABEL_FOLDER).replace(os.path.splitext(x)[-1], '.txt')
                            for x in self.img_files]
        print (self.label_files[:1])
        ...
        if cache_labels or image_weights:  # cache labels for faster training
            ...
            self.labels_drop = [np.zeros((0, 6))] * n
            ...
            for i, file in enumerate(pbar):
                ...
                if l.shape[0]:
                    ...
                    self.labels[i] = l
                    self.labels_drop[i] = np.c_[i * np.ones(l.shape[0]), l]
                    nf += 1  # file found
        ...
        print ( [list(np.concatenate(self.labels,axis=0)[:,0]).count(i) for i in range(12)])
        ...
        if keep_rate is not None or keep_rate < 1:
            self.img_files, self.labels_drop = discard_label(labels=self.labels_drop, imgs = self.img_files, classes= np.unique(np.concatenate(self.labels_drop,axis=0)[:,1]), rate=keep_rate)

            self.labels = self.labels_drop




    def __len__(self):
        print ('Size of dataset %d'%len(self.img_files))
...
def discard_label(labels, imgs, classes, rate):
    # import pdb;pdb.set_trace()
    temp_label = np.concatenate(labels, axis=0)
    final_cls_index = []
    for c in classes:
        index = np.where(temp_label[:, 1] == c)[0]
        np.random.shuffle(index)
        final_cls_index.append(index[: int(len(index) * rate)])

    final_cls_index= np.concatenate(final_cls_index ,axis=0).flatten()

    index_img = np.unique(temp_label[final_cls_index, 0 ]).astype('int')

    temp_label = temp_label[final_cls_index]

    final_label = []
    final_imgs = []
    for img_i in index_img:
        final_label.append(temp_label[temp_label[:,0]==img_i,1:])
        final_imgs.append(imgs[img_i] )
