...
from utils import custome_datasets
from utils.utils import *
...
def train():
    ...
    wdir = 'weights_%s_%g_alpha_%g_beta_%g'%(Exp_name, KR_sample, opt.alpha, opt.beta )+ os.sep  # weights dir
    if not os.path.isdir(wdir):
        os.mkdir(wdir)
    last = wdir + 'last.pt'
    best = wdir + 'best.pt'
    results_file = 'results_%s_%g.txt'%(Exp_name, KR_sample)
    ...
    lable_path = data_dict['label_train']
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        # load model
        # if opt.transfer:
        print ('Load pre-trained models at %s'%weights)
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[22,40,150], gamma=0.1)
    ...
    LPI_D_1 = custome_datasets.incld_or_excld_dataset(data_setup[0], flag_incld = True, path=None, img_size=416)
    LPI_D_2 = custome_datasets.incld_or_excld_dataset(data_setup[1], flag_incld=True, path=None, img_size=416)
    dataset = ConcatDataset([LPI_D_1,LPI_D_2])
    # Dataloader
    ...
    # ----- OD_LPI_D1 and OD_LPI_2 are loaded ------
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        model.train()
        for g in optimizer.param_groups:print('Learning Rate ',g['lr'])

        print(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'GIoU', 'obj', 'cls', 'total', 'targets', 'img_size'))

        # Freeze backbone at epoch 0, unfreeze at epoch 1 (optional)f
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            loss, loss_items = modified_compute_loss(imgs, pred, targets, roi, model, A_CNN, alpha = opt.alpha, beta=opt.beta, device=device,
                                                         epoch=epoch, epoch_threshold=1, kmeans=kmeans, img_path= paths)
        ...
        if save:
            ...
            if epoch > 0 and epoch % 10 == 0:
                torch.save(chkpt, wdir + 'backup%d.pt' % (epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha', type=float, help= 'contoling the effect of pseudo labels in training')

    parser.add_argument('--beta', type=float, help = 'interpolating between YOLO class-prediction and A-CNN prediction')
